...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitleColor:kColorOrangeRed forState:UIControlStateNormal];
        [rightBtn setTitleColor:[UIColor grayColor] forState:UIControlStateDisabled];
        rightBtn.enabled = YES;
        rightBtn.titleLabel.font = [UIFont systemFontOfSize:16 * kAUTOLAYOUTSCALE];
        rightBtn.frame = CGRectMake(0, 0, 45 * kAUTOLAYOUTSCALE, 45* kAUTOLAYOUTSCALE);
        rightBtn.titleEdgeInsets = UIEdgeInsetsMake(0, 10* kAUTOLAYOUTSCALE, 0, 0);
...
- (UICollectionView *)toolBarThumbCollectionView{
    if (!_toolBarThumbCollectionView) {
        ...
        UICollectionView *toolBarThumbCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(10, 0, self.view.zl_width - 120, 44) collectionViewLayout:flowLayout];
...
- (ZLPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        collectionView.contentInset = UIEdgeInsetsMake(0, 0,TOOLBAR_HEIGHT, 0);
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.frame = CGRectMake(-20 * kAUTOLAYOUTSCALE , 13 * kAUTOLAYOUTSCALE, 20 * kAUTOLAYOUTSCALE, 20 * kAUTOLAYOUTSCALE);
        makeView.hidden = YES;
        makeView.layer.cornerRadius = makeView.frame.size.height / 2.0;
        makeView.clipsToBounds = YES;
        makeView.backgroundColor = kColorOrangeRed;
...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationItem.leftBarButtonItem.tintColor = kColorOrangeRed;
    self.navigationController.navigationItem.title = @"选择照片";
    // 初始化按钮
    [self setupButtons];
...
- (void) setupToorBar{
    ...
    [toorBar setBackgroundColor:[UIColor whiteColor]];
...
- (void)setAssetsGroup:(ZLPhotoPickerGroup *)assetsGroup{
    ...
    if ([assetsGroup.groupName isEqualToString:@"相机胶卷"]) {
        self.title = @"选择照片";
    }else
    {
