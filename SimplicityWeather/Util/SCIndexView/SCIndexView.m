...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:kSCCenterStringFromSelector] || [keyPath isEqualToString:kSCFrameStringFromSelector]) {
        ...
        [CATransaction setDisableActions:YES];
        if (self.searchLayer) {
            self.searchLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight - 5, SCGetTextLayerCenterY(0, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight + 10, self.configuration.indexItemHeight);
            self.searchLayer.cornerRadius = self.configuration.indexItemHeight / 2;
            self.searchLayer.contentsScale = UIScreen.mainScreen.scale;
            self.searchLayer.backgroundColor = self.configuration.indexItemBackgroundColor.CGColor;
        }
        ...
        for (int i = 0; i < self.subTextLayers.count; i++) {
            CATextLayer *textLayer = self.subTextLayers[i];
            NSUInteger section = i + deta;
            NSInteger spaceWidth = self.dataSource[section].length > 1 ? 10 : 0;
            textLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight - spaceWidth / 2, SCGetTextLayerCenterY(section, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight + spaceWidth, self.configuration.indexItemHeight);
        }
        [CATransaction commit];
...
- (UILabel *)indicator
{
    if (!_indicator) {
        ...
        _indicator.adjustsFontSizeToFitWidth = YES;
