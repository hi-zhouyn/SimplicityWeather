...
- (void)buildWaterDropView
{
    ...
    YAWaveView *waveView = [[YAWaveView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
    [self.contentView addSubview:waveView];
    [waveView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.height.mas_equalTo(100);
        make.top.equalTo(self.contentView).offset((self.frame.size.height - 100 - 28) / 2 + 28);
    ...
    [_numLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(waveView.mas_bottom).offset(20);
        make.centerX.equalTo(waveView);
    }];
